...
import com.fengruyi.common.entity.HttpRequest;
import com.fengruyi.common.entity.HttpResponse;
...
public class HttpUtils {
    ...
    public static String getUrlWithParas(String url, Map<String, String> parasMap) {
        StringBuilder urlWithParas = new StringBuilder(TextUtil.isEmpty(url) ? "" : url);
    ...
    public static String joinParasWithEncodedValue(Map<String, String> parasMap) {
        ...
        if (parasMap != null && parasMap.size() > 0) {
            ...
            try {
                while (ite.hasNext()) {
                    Map.Entry<String, String> entry = (Map.Entry<String, String>)ite.next();
                    paras.append(entry.getKey()).append(EQUAL_SIGN).append(TextUtil.utf8Encode(entry.getValue()));
                    if (ite.hasNext()) {
                        paras.append(PARAMETERS_SEPARATOR);
                    }
                }
    ...
    public static void setURLConnection(Map<String, String> requestProperties, HttpURLConnection urlConnection) {
        if (TextUtil.isEmpty(requestProperties) || urlConnection == null) {
            return;
        }

        for (Map.Entry<String, String> entry : requestProperties.entrySet()) {
            if (!TextUtil.isEmpty(entry.getKey())) {
    ...
    private static void setHttpResponse(HttpURLConnection urlConnection, HttpResponse response) {
        ...
        response.setResponseHeader(AppConstant.EXPIRES, urlConnection.getHeaderField("Expires"));
        response.setResponseHeader(AppConstant.CACHE_CONTROL, urlConnection.getHeaderField("Cache-Control"));
    }
    ...
    private static class HttpRequestAsyncTask extends AsyncTask<HttpRequest, Void, HttpResponse> {
        ...
        protected HttpResponse doInBackground(HttpRequest... httpRequest) {
            if (TextUtil.isEmpty(httpRequest)) {
                return null;
            }
